...
class SequentialSampler(Sampler):
    ...
    """
    def __init__(
        self, dataset_len, epoch_size=None, init_sample_probs=None,
        sample_probs_power=1.5, replace=True,
    ):
        ...
        if init_sample_probs is None:
            self.init_sample_probs = \
                np.array([1000. for i in range(dataset_len)], dtype=np.float64)
        else:
            self.init_sample_probs = \
    ...
    def _sample_probs_normalization(self, sample_probs):
        """Probabilities normalization to make the sum equal to 1.
        The sum might not be equal exactly to 1 if probs are too small.
        """
...
def collate_fn(batch):
    images, texts, enc_texts, idxs = zip(*batch)
    images = torch.stack(images, 0)
    text_lens = torch.LongTensor([len(text) for text in texts])
    enc_pad_texts = pad_sequence(enc_texts, batch_first=True, padding_value=0)
    return images, texts, enc_pad_texts, text_lens, idxs
...
class OCRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return image, text, enc_text, idx
